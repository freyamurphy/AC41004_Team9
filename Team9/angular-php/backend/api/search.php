...
#Get the product table from the database
    $sql = "SELECT * FROM providers WHERE City LIKE '%$search%'  OR ID LIKE '%$search%' OR providerName LIKE '%$search%' OR StreetAddress LIKE '%$search%'  OR ZipCode LIKE '%$search%' LIMIT 10";
}

if($search!=''&&$state!=''){  $sql = "SELECT * FROM providers WHERE State='$state' AND ( City LIKE '%$search%' OR ID LIKE '%$search%' OR providerName LIKE '%$search%' OR StreetAddress LIKE '%$search%'  OR ZipCode LIKE '%$search%') LIMIT 10";
}
